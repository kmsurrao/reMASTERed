...
ells = np.arange(ellmax+1)
plt.clf()
plt.plot(ells[0:], (ells*(ells+1)*term1/(2*np.pi))[0:], label='term1 contribution (original MASTER equation)')
plt.plot(ells[0:], (ells*(ells+1)*term2/(2*np.pi))[0:], label='term2 contribution', linestyle='dotted')
# plt.plot(ells[0:], (ells*(ells+1)*term3/(2*np.pi))[0:], label='term3 contribution')
plt.plot(ells[0:], (ells*(ells+1)*masked_map_cl/(2*np.pi))[0:], label='LHS of MASTER equation')
plt.plot(ells[0:], (ells*(ells+1)*master_cl/(2*np.pi))[0:], label='RHS of MASTER equation with new terms', linestyle='dotted')
plt.legend()
plt.xlabel(r'$\ell$')

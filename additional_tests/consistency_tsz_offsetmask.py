...
def get_one_map_and_mask(inp, sim, offset, base_dir):
    '''
    PARAMETERS
    inp: Info object, contains input specifications
    sim: int, simulation number
    offset: float, offset = mask-map
    base_dir: str, directory containing tSZ maps
...
if __name__=='__main__':
    ...
    #define offset A for mask W=a+A
    offset = 1.e-6

    #get all maps and masks
    pool = mp.Pool(min(inp.nsims, 16))
    results = pool.starmap(get_one_map_and_mask, [(inp, sim, offset, base_dir) for sim in range(inp.nsims)])
    pool.close()
    ...
    pickle.dump(Rho, open(f'rho/rho_tsz_offsetmask_ellmax{inp.ellmax}.p', 'wb')) #remove
    # Rho = pickle.load(open(f'rho/rho_tsz_offsetmask_ellmax{inp.ellmax}.p', 'rb')) #remove


    #test reMASTERed
    print('Testing reMASTERed', flush=True)
    compare_master(inp, master_lhs, w00, a00, Cl_aa, Cl_ww, Cl_aw, bispectrum_aaw, bispectrum_waw, Rho, my_env, base_dir=f'images/tSZ_w_eq_a_plus_A_ellmax{inp.ellmax}_{inp.nsims}sims')
